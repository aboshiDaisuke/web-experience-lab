...
トップは制作依頼につなげるためのポートフォリオです。ヒーロー（ブラウザで動く熱帯魚の水槽）→ 代表作（「ぜんぶ、ブラウザの中で動いています。」。作品数・リアルタイム3Dの数などの数字と、代表作3つ。カーソルを乗せると実際のページがタイルの中で動く）→ 作品一覧（目的別の絞り込み・技術タグ付き。最初の9作品のあと「すべて見る」）→ できること → 料金の目安 → 制作の流れ・FAQ → 相談フォーム、の順に並びます。各作品ページ右下からも、その作品を引き継いで相談フォームへ移動できます（`/?ref=<slug>#contact`）。

つくり方の比較（WordPress・Astra・STUDIO・Next.js・Shopify・HTML）、準備状況と費用、制作できるかの事前チェック、オプション料金・見積り例は、依頼ガイド `/guide` にまとめています（トップのヘッダーと料金欄から移動）。事前チェックの回答は、ガイド下部の相談フォームに引き継がれます。
...
### 新型車・通販・ゲーム会社（2026-09 追加）

同じく自己完結型。本体は `components/works/<slug>-site.tsx`（`project-experience.tsx` から React.lazy で読み込み）、スタイルは本体の先頭で `app/signatures/<slug>.css` を読み込みます。

| 作品 | 仕掛け |
|---|---|
| MIRAI ARC（架空の新型電気自動車） | Blenderで作った車（`tools/blender/arc.py` → `arc_build.sh` で `public/models/arc/` に Draco 圧縮のGLB）をスタジオで回し、ドア・ライト・充電口・ホイールに触れて動かせる。「運転席に乗りこむ」でカメラが運転席の目線へ入り、メーターとセンターディスプレイ（Canvas描画）が起動。色・ホイール・内装・アンビエントライト、充電時間の計算、グレード・補助金・月々の支払いの試算から試乗予約へ |
| MIRAI GOODS（暮らしの道具の通販） | カートが段ボール箱（three.js）。入れた品が箱に落ちて詰まり（自前の箱詰め計算と落下アニメーション）、箱のサイズ（60〜140）と送料がその場で決まる。割れ物は紙のクッションとワレモノシール、ギフトは包装とリボン。品物15点はすべてコードで生成した3D（轆轤の形、釉薬・木目のシェーダー）。一覧の絞り込み、商品詳細、送料表、購入手続きのデモ |
| MIRAI GAMES（ゲーム会社） | トップがそのまま遊べるランナーゲーム「MIRAI DASH」（three.js、ブルーム・CRT風のポストエフェクト、ハイスコア保存）。タイトル一覧は3Dのソフトパッケージで、回して開くとディスクと説明書が出る。採用はキャラクター選択画面（能力値のレーダー、仕事内容、1日の流れ）。ニュース、会社情報 |

3作品とも画像素材は使わず、3D・絵はすべてコード（three.js・シェーダー・Canvas）か Blender で自作しています。
...
- `npx tsc --noEmit`
- `npm run build`
- Playwright（ヘッドレスChromium）で、トップ・依頼ガイド・追加作品を 1440×900 と 390×844 で撮影し、主要な操作を実行して確認（GPUなしのソフトウェア描画。実機のGPUでの速度は未確認）
